...
export default function ChatBox() {
  ...
  const messages = [
    ...
    {
      from: "agent",
      text: "Sure! We provide web design, development, and IT consulting.",
    },
  ];
    ...
    <div className="fixed bottom-20 right-6 z-50 flex flex-col items-end">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="bg-[#47919F] text-white px-4 py-2 font-semibold flex justify-between items-center">
              Chat with us
              <button
              ...
              {messages.map((msg, idx) => (
                <div
                  ...
                  className={`px-3 py-2 rounded-lg max-w-[80%] break-words ${
                    msg.from === "agent"
                      ? "bg-[#e0f2f5] self-start text-[#1f2937]"
                      : "bg-[#47919F] text-white self-end"
                  }`}
                >
                  {msg.text}
                </div>
            ...
            <div className="px-4 py-2 border-t border-gray-200">
              <input
                type="text"
                placeholder="Type a message..."
                className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#47919F]"
                disabled
              />
            </div>
          </motion.div>
